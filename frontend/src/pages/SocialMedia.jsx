...
export default function SocialMedia() {
  ...
  useEffect(() => {
    // Each section loads and fails independently — a broken comparison
    // fetch should never blank out the rest of the page, and vice versa.
    api
      .get("/social/platforms")
      .then((res) => setPlatforms(res.data.data ?? []))
      .catch(() => setError("Couldn't load platform data."))
                ...
                <tbody className="divide-y divide-slate-100">
                  {performance.map((row) => (
                    ...
                    >
                      ...
                      <td className="text-right px-4 py-4 text-slate-600">
                        {formatNumber(row.total_comments)}
                      </td>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <Metric
                      icon={<Eye size={14} />}
                      label="Views"
                      value={formatNumber(row.total_views)}
                    />

                    <Metric
                      icon={<Heart size={14} />}
                      label="Likes"
                      value={formatNumber(row.total_likes)}
                    />

                    <Metric
                      icon={<MessageCircle size={14} />}
                      label="Comments"
                      value={formatNumber(row.total_comments)}
                    />

                    <Metric
                      icon={<Radio size={14} />}
                      label="Reach"
                      value={formatNumber(row.total_reach)}
                    />
                  </div>
                </div>
              ))}
