...
def get_all_audience(creator_id: int | None = None, db: Session = Depends(get_db)):
    query = db.query(Audience)
    if creator_id is not None:
        query = query.filter(Audience.creator_id == creator_id)

    records = [
        AudienceResponse.model_validate(r).model_dump() for r in query.all()
    ]
...
def update_audience(audience_id: int, payload: AudienceUpdate, db: Session = Depends(get_db)):
...
def audience_demographics(creator_id: int, db: Session = Depends(get_db)):
    return get_audience_demographics(db, creator_id)


@analytics_router.get("/audience/{creator_id}/locations")
def audience_locations(creator_id: int, db: Session = Depends(get_db)):
    return get_top_locations(db, creator_id)


@analytics_router.get("/audience/{creator_id}/active-hours")
def audience_active_hours(creator_id: int, db: Session = Depends(get_db)):
