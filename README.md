...
Create your first user via `POST /users/`, then log in from the frontend.
...
- **Other platforms** (Instagram, TikTok, Facebook, LinkedIn, X) use generated mock data
  ...
  per-platform developer approval. Metrics a platform doesn't actually support are
  stored as `0`/`NULL`, never invented.
