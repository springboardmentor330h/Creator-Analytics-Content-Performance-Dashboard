...
class UserService:
    ...
    def create(db: Session, user_in: UserCreate) -> User:
        ...
        # Bootstrap rule: as long as the system has no Administrator yet, the
        # next account created becomes one automatically. Without this there
        # is no way to reach the admin panel at all — every other signup path
        # (including the public registration form) only ever creates
        # non-admin roles, and this also covers databases that were seeded
        # with demo creators but no admin.
        has_admin = (
            db.query(User)
            .filter(User.role == UserRole.ADMINISTRATOR, User.is_deleted == False)  # noqa: E712
            .first()
            is not None
        )
        role = user_in.role if has_admin else UserRole.ADMINISTRATOR
