...
def register(data:UserRegister,db:Session=Depends(get_db)):
    ...
    u=User(full_name=data.full_name,email=data.email,hashed_password=hash_password(data.password),
           role=data.role,bio=data.bio,is_active=True,is_deleted=False)
    db.add(u); db.commit(); db.refresh(u)
    return {"message":"User registered successfully","user_id":str(u.id)}
