  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">Generate summaries and download PDF or Excel</p>
      </div>
      {error && <ErrorBox message={String(error)} />}

      <div className="flex flex-wrap gap-3 items-end">
        <div>
          <label className="text-xs text-slate-500 dark:text-slate-400">Report type</label>
          <select
            value={type}
            onChange={(e) => setType(e.target.value)}
            className="block mt-1 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm dark:bg-slate-950 dark:border-slate-800"
          >
            {TYPES.map((t) => (
      ...
      {report && (
        <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-3 dark:bg-slate-900 dark:border-slate-800">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
            {Object.entries(summary)
              .filter(([, v]) => v !== null && v !== undefined && typeof v !== 'object')
              .map(([k, v]) => (
                <div key={k} className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 dark:bg-slate-950 dark:border-slate-800">
                  <p className="text-xs text-slate-500 capitalize dark:text-slate-400">{k.replaceAll('_', ' ')}</p>
