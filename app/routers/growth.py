...
def create_growth_record(payload: GrowthCreate, db: Session = Depends(get_db)):
...
def get_all_growth(creator_id: int | None = None, db: Session = Depends(get_db)):
...
def delete_growth_record(growth_id: int, db: Session = Depends(get_db)):
...
def growth_trend(creator_id: int, platform: str | None = None, db: Session = Depends(get_db)):
    return get_growth_trend(db, creator_id, platform)


@analytics_router.get("/growth/{creator_id}/platform-comparison")
def growth_platform_comparison(creator_id: int, db: Session = Depends(get_db)):
