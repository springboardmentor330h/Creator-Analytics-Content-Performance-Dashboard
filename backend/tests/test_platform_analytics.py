...
def test_tiktok_is_labeled_as_mock(client):
    """TikTok has no data source (live or sample) yet, so it stays mock."""
    headers = get_auth_headers(client)
    response = client.get("/api/platforms/tiktok", headers=headers)
...
def test_instagram_is_labeled_as_real_not_mock(client):
    """Instagram now has realistic sample data seeded into PostgreSQL
    (see instagram_sample_data_service.py) -- it graduated from the
    mock fallback the same way YouTube did in Sprint 5, via the
    REAL_DATA_PLATFORMS extension point."""
    headers = get_auth_headers(client)
