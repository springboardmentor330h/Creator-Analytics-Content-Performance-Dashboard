...
def dashboard_overview(creator_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "audience": {
            "followers": latest_audience.followers if latest_audience else 0,
            "impressions": latest_audience.impressions if latest_audience else 0,
            "reach": latest_audience.reach if latest_audience else 0,
        },
        "revenue": {
